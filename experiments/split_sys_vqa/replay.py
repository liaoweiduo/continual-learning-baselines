...
def er_ssysvqa_ti(override_args=None):
    """
    ...
    for experience in benchmark.train_stream:
        ...
        cl_strategy.train(experience)   # only eval self
...
def er_ssysvqa_ci(override_args=None):
    """
    ER algorithm on split systematic VQA on class-IL setting.
    """
    args = create_default_args({
        'cuda': 0, 'seed': 0,
...
if __name__ == '__main__':
    ...
    if args.setting == 'task':
        res = er_ssysvqa_ti(vars(args))
    elif args.setting == 'class':
        res = er_ssysvqa_ci(vars(args))
    else:

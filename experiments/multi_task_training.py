...
def multi_task_training(override_args=None):
    ...
    # BENCHMARK
    # ####################
    benchmark = get_benchmark(args, multi_task=True)
    ...
    if strategy is None:
        ...
        # MODEL
        # ####################
        model = get_model(args, multi_task_baseline=True)
    ...
    if not args.do_not_store_checkpoint_per_exp:
        model_file = os.path.join(checkpoint_path, f'model-0.pth')

...
class BasicBlockFilm(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, multi_head=1):
        ...
        self.film1 = nn.ModuleList([CatFilm(planes) for _ in range(multi_head)])
        self.film2 = nn.ModuleList([CatFilm(planes) for _ in range(multi_head)])
    ...
    def forward(self, x, head_idx=0):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.film1[head_idx](out)
        out = self.relu(out)

        out = self.conv2(out)
        out = self.bn2(out)
        out = self.film2[head_idx](out)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, classifier=None, num_classes=None,
        ...
        # todo: apply multi_head for sharing the same backbones
